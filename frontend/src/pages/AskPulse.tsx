...
const AskPulse = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-start pt-12 sm:justify-center sm:pt-0 min-h-[calc(100vh-8rem)] animate-fadeIn px-4">
        <div className="text-center mb-8 w-full">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-3 text-center">
        ...
        <div className="w-full max-w-2xl relative group">
          ...
          <input
            ...
            className="block w-full pl-11 pr-14 py-3.5 bg-white border border-slate-200 rounded-full text-base shadow-sm focus:ring-4 focus:ring-blue-50 focus:border-blue-400 transition-all outline-none"
          />
          <button
        ...
        <div className="mt-8 w-full max-w-3xl">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 text-center">Suggested Categories</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {CATEGORIES.map((cat, idx) => (
              <button
                key={idx}
                onClick={() => handleSearch(cat.prompt)}
                className="flex flex-col items-center justify-center p-4 bg-white border border-slate-100 rounded-xl hover:border-blue-300 hover:shadow-md transition-all group"
              >
                <cat.icon className="w-5 h-5 text-slate-400 group-hover:text-blue-500 mb-2 transition-colors" />
                <span className="text-xs font-medium text-slate-700 group-hover:text-blue-700 text-center">{cat.label}</span>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="max-w-5xl mx-auto min-h-[calc(100vh-8rem)] flex flex-col animate-fadeIn pb-12">
      {/* Top Search Bar */}
      <div className="sticky top-0 bg-[#fcfcfc] z-10 pt-4 pb-6 border-b border-slate-100 mb-8">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex-1 w-full relative">
            ...
            <input
              ...
              onChange={(e) => setQuery(e.target.value)}
              onKeyDown={handleKeyDown}
              className="block w-full pl-10 pr-12 py-3 bg-white border border-slate-200 rounded-full text-sm shadow-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-all outline-none"
            />
            <button
              onClick={() => handleSearch(query)}
      ...
      <div className="px-2">
        <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">
          ...
          <span className="font-medium text-slate-600">Results for</span> "{activeSearch.query}"
        </h2>
        ...
        {activeSearch.isLoading ? (
          <div className="space-y-4">
            ...
            <div className="h-4 bg-slate-200 rounded-md animate-pulse w-1/2"></div>
          </div>
        ) : (
          <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
            <div className="prose prose-blue max-w-none prose-p:leading-relaxed prose-headings:font-bold text-slate-800 text-lg">
