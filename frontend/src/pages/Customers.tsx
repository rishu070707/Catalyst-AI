    ...
    <div className="max-w-6xl mx-auto pb-12">
      <div className="flex flex-col items-center justify-center mb-8 gap-4 relative text-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Unified view of your retail customer base.</p>
        </div>
      ...
      <div className="bg-white border border-border-default rounded-lg shadow-sm overflow-hidden animate-fadeIn">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-surface-secondary text-xs uppercase text-gray-500 font-semibold border-b border-border-default">
              <tr>
            ...
            <tbody className="divide-y divide-border-default">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-400">No customers match that segment.</td>
                </tr>
              ) : (
                (Array.isArray(data) ? data : (data?.customers || [])).map((customer) => (
                  <React.Fragment key={customer.id}>
                    <tr className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => navigate(`/customers/${customer.id}`)}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500 mt-0.5">ID: {customer.id.substring(0, 8)}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">{customer.orderCount} Orders</div>
                      </td>
                      <td className="px-6 py-4">
        ...
        {data && data.totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-border-default bg-gray-50">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 border border-border-default rounded text-sm font-medium hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
    ...
    <div className="p-6 text-sm text-gray-700">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h4 className="font-semibold text-gray-900 mb-2">AI Customer Narrative</h4>
          {isLoading ? (
