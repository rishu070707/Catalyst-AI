...
const EmptyState = ({ title, description, action }: any) => (
  <div className="text-center p-10 bg-white border border-slate-200 rounded-xl">
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 page-enter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col items-center text-center gap-4 relative overflow-hidden">
            {/* Initials Avatar */}
            ...
            <div>
              ...
              <div className="flex justify-center gap-1.5 items-center mt-1.5 flex-wrap">
                <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase bg-slate-100 text-slate-600 border border-slate-200 font-mono">
            ...
            {/* Details List */}
            <div className="w-full flex flex-col gap-2.5 text-left border-t border-slate-100 pt-4 text-xs text-slate-600">
              {customer.email && (
                <div className="flex items-center gap-2">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Value Metrics</h3>
            <div className="space-y-3">
              <div>
                <span className="text-[10px] text-slate-500 block">LIFETIME VALUE</span>
                <span className="text-xl font-bold text-emerald-600 font-mono">₹{ltv.toLocaleString()}</span>
              </div>
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                <div>
                  <span className="text-[9px] text-slate-400 block uppercase">Orders</span>
                  <span className="text-sm font-bold text-slate-900 font-mono">{orderCount}</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-400 block uppercase">Avg Spend</span>
                  <span className="text-sm font-bold text-slate-900 font-mono">₹{Math.round(customer.avg_order_value || (customer.totalSpent / Math.max(orderCount, 1)) || 0).toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Churn Risk Radar</h3>
            ...
            <div className="flex items-center justify-between gap-4 pt-1">
              <div className="flex flex-col">
                <span className="text-2xl font-bold text-slate-900 font-mono">{customer.churn_risk_score}%</span>
                <span className="text-[10px] text-slate-400">Churn risk estimate</span>
              </div>
              <div className="relative w-12 h-12 flex items-center justify-center">
                ...
                <span className="absolute text-[9px] font-bold text-slate-900 font-mono">{customer.churn_risk_score}%</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[10px] bg-slate-50 text-slate-500 border border-slate-200 px-2 py-0.5 rounded font-mono font-bold">
                {ordersData?.total_orders || 0} Orders
              </span>
            </div>

            {isOrdersLoading ? (
              <div className="py-8 text-center text-xs text-slate-400 animate-pulse">Loading order history...</div>
                  ...
                  <tbody>
                    {ordersData.orders.map((order: any) => (
                      <tr key={order.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        <td className="py-2.5 font-bold font-mono text-slate-900">ORD-{order.id.slice(0, 5).toUpperCase()}</td>
                        <td className="py-2.5 text-slate-500">{new Date(order.created_at || order.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                        <td className="py-2.5 text-right font-bold text-slate-900 font-mono">₹{order.amount.toLocaleString()}</td>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            <h3 className="font-bold text-slate-900 text-xs">Campaign & Interaction Timeline</h3>
              ...
              <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                {timelineEvents.map((event, idx) => (
                  <div key={idx} className="relative">
                    {/* Icon Node */}
                    <div className={`absolute -left-[23px] top-0.5 w-5 h-5 rounded-full flex items-center justify-center border border-slate-200 bg-white text-slate-500 shadow-sm ${event.color}`}>
                      {event.icon}
                    </div>
                    <div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-slate-900 text-xs">AI Customer Narrative Profile</h3>
            </div>

            {isStoryLoading ? (
              <div className="py-4 text-center text-xs text-slate-400 animate-pulse">Drafting story...</div>
            ) : storyData ? (
              <p className="text-xs leading-relaxed text-slate-600 italic bg-blue-50/40 border border-blue-100 p-3.5 rounded-lg">
                "{storyData.story}"
              </p>
            ) : (
              <p className="text-xs text-slate-400">Failed to generate summary narrative.</p>
            )}
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <select
                ...
                className="bg-white border border-slate-200 rounded px-2 py-1 text-[11px] font-semibold text-slate-700 focus:outline-none focus:border-blue-500"
              >
            ...
            {isExplainLoading ? (
              <div className="py-6 text-center text-xs text-slate-400 animate-pulse">Analyzing attributes...</div>
            ) : explanationData ? (
              <ExplainabilityCard
              ...
              <div className="text-center py-6 text-xs text-slate-400">No targeting metrics loaded.</div>
            )}
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-4 gap-2 text-center text-[10px]">
                <div className="p-2 border border-slate-200 rounded bg-slate-50 font-semibold text-slate-700">
                  Shopify
                  <div className="text-[8px] font-mono text-slate-400 mt-1 truncate">{customer.email ? 'Email Match' : 'N/A'}</div>
                </div>
                <div className="p-2 border border-slate-200 rounded bg-slate-50 font-semibold text-slate-700">
                  POS
                  <div className="text-[8px] font-mono text-slate-400 mt-1 truncate">{customer.phone ? 'Phone Match' : 'N/A'}</div>
                </div>
                <div className="p-2 border border-slate-200 rounded bg-slate-50 font-semibold text-slate-700">
                  Loyalty
                  <div className="text-[8px] font-mono text-slate-400 mt-1 truncate">LOY-{customer.id.slice(0,4).toUpperCase()}</div>
                </div>
                <div className="p-2 border border-slate-200 rounded bg-slate-50 font-semibold text-slate-700">
                  CSV
                  <div className="text-[8px] font-mono text-slate-400 mt-1 truncate">Imported</div>
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-50 border border-slate-100 rounded-lg space-y-2 text-xs">
                <div className="flex justify-between text-slate-500 font-medium">
                  ...
                  <span className="font-bold text-slate-900 font-mono">97%</span>
                </div>
                <div className="flex justify-between text-slate-500 font-medium">
                  <span>Stitching Reason:</span>
                  <span className="text-slate-900 font-bold text-right">Matching Phone & Email</span>
                </div>
                <div className="flex items-center gap-1 text-[10px] text-emerald-600 font-semibold pt-1 border-t border-slate-200">
